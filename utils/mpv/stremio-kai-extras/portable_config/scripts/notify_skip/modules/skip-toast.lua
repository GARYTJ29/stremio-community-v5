...
function SkipToast:update_dimensions()
    ...
    local has_hint = self.hint_text ~= nil and self.hint_text ~= ""
    local margin = 80 * self.scale
    local button_width = (has_hint and 240 or 200) * self.scale
    local button_height = (has_hint and 78 or 60) * self.scale
...
function SkipToast:render()
    ...
    if has_hint then
        -- Main text, nudged up to make room for the hint below it
        ass:txt(x, round(y - 15 * self.scale), 5, self.message, {
            size = font_size * 0.85,
            color = text_color,
            opacity = 0.9,
            bold = true,
        ...
        -- Key hint (e.g. "Press Tab" / "RB+A")
        ass:txt(x, round(y + 16 * self.scale), 5, self.hint_text, {
            size = font_size * 0.5,
            color = text_color,
            opacity = 0.55,

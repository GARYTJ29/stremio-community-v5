...
function M.show_skip_overlay(message, duration, is_prompt, hint_text)
    ...
    -- Prompts get a key hint if the caller didn't supply one already (e.g. the
    -- confirmation prompt, which appends "to confirm"). Non-prompt messages (e.g.
    -- "Skip Cancelled") get an explicit empty hint - skip_toast:set_message()
    -- would otherwise fall back to its own default and show one anyway.
    if is_prompt then
        hint_text = hint_text or (state.ui_state.gamepad_active and "RB+A" or "Press Tab")
    else

...
local function show_confirmation_prompt(log_message)
    ...
    if M.notification then
        local hint = state.ui_state.gamepad_active and "RB+A to confirm" or "Press TAB to confirm"
        M.notification.show_skip_overlay("Skip?", config.CONSTANTS.CONFIRMATION_TIMEOUT, true, hint)
    end

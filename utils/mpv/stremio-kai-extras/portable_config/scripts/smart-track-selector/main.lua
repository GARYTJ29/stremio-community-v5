...
local config = {
    -- Master switch. The shell's "Smart Track Selection" toggle drives this at
    -- runtime; when off the script leaves aid/sid entirely to mpv and Stremio.
    enabled = true,
...
local function on_file_loaded()
    ...
    if not config.enabled then
        state.selection_phase = "user"
        trace("selection-skipped", { branch = "file-loaded", reason = "disabled" })
        return
    end
    state.selection_phase = "automatic"
...
local function update_config(json_data)
    ...
    if new_config.enabled ~= nil then
        config.enabled = new_config.enabled
        log_debug("  Updated enabled: " .. tostring(config.enabled))
    end
    ...
    if not config.enabled then
        ...
        state.external_sub_watch = false
        if state.external_sub_timer then
            trace("timer-cancelled", { timer_id = state.external_sub_timer_id, kind = "external-subtitle-timeout", reason = "disabled" })
            state.external_sub_timer:kill()
            state.external_sub_timer = nil
            state.external_sub_timer_id = nil
        end
        state.selection_phase = "user"
        trace("config-reevaluation-skipped", { reason = "disabled" })
        log_info("Smart track selection disabled; leaving tracks to mpv/Stremio")
        finish_selection_run(run_id, "disabled", {})
        return
    end
